...
//#pragma omp parallel for firstprivate(t_x,t_y) lastprivate(p_x,p_y)  num_threads(4) //collapse(4) num_threads(4)  //reduction(+:global_histogram) num_threads(4) collapse(2)
//#pragma parallel for num_threads(2)
    for (unsigned int t_x = 0; t_x < cpu_TILES_X; ++t_x) {//unsigned int
...
#pragma omp parallel for lastprivate(t_x,t_y) collapse(2) num_threads(4)
        for ( t_x = 0; t_x < cpu_TILES_X; ++t_x) {//unsigned int
...
void openmp_stage3() {
    // Calculate interpolated pixels from normalised histograms
    {
        ...
        long long p_x, p_y;
        // For each tile
#pragma omp parallel for firstprivate(p_x,p_y) lastprivate(t_x,t_y) num_threads(4) ///for //collapse(2)
        for ( t_x = 0; t_x <cpu_TILES_X; ++t_x) {
            for ( t_y = 0; t_y < cpu_TILES_Y; ++t_y) {//int
                const unsigned int tile_offset = (t_y * cpu_TILES_X * TILE_SIZE * TILE_SIZE + t_x * TILE_SIZE);
                // For each pixel within the tile
                for ( p_x = 0; p_x < TILE_SIZE; ++p_x) {//unsigned
